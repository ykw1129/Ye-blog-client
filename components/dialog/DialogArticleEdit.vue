...
export default class DialogArticleEdit extends Mixins(UploadMixin, CaptchaMixin) {
  @State auth: any
  @Prop({ type: Boolean, required: true }) private ArticleEditVisible!: boolean
  @Prop({ type: Object, required: true }) private ArticleConent:any
  ...
  // 初始化渲染表单
  init () {
    this.articleForm.id = this.ArticleConent._id
    this.articleForm.title = this.ArticleConent.title
    this.articleForm.coverId = this.ArticleConent.coverId._id
    this.articleForm.description = this.ArticleConent.description
    this.imageUrl = this.ArticleConent.coverId.url
    this.tags = this.ArticleConent.tags
    this.articleForm.tags = this.ArticleConent.tags.map((tag:any) => tag._id)
    this.articleForm.status = this.ArticleConent.status
    this.articleForm.contentType = this.ArticleConent.contentType
    if (this.ArticleConent.contentType === '1') {
      this.articleForm.markContent = this.ArticleConent.content
    } else {
      this.articleForm.content = this.ArticleConent.content
    }
  }
  ...
  putArticle () {
    ...
    this.articleRef.validate(async (valid: boolean) => {
      if (valid) {
        ...
        const res = await putUserArticle({
          $axios: this.$axios,
          param:
          {
